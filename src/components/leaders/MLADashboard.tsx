...
const MLADashboard: React.FC = () => {
  ...
  const checkAccess = async () => {
    ...
    try {
      ...
      // Check if user is specifically an MLA
      const leaderTypes = await getUserLeaderTypes(user.id);
      if (!leaderTypes.includes('mla')) {
        toast({
          title: "Access Denied",
          description: "You are not assigned as an MLA for any community",
          variant: "destructive"
        });
        navigate('/');
        return;
      ...
      console.error('Error checking MLA access:', error);
      navigate('/');
    }
  };

  return <LeaderDashboard leaderType="mla" />;
};
